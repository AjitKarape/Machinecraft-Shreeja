...
export default function CostSummary() {
  ...
  useEffect(() => {
    ...
    const mappingsChannel = supabase
      .channel("expense-mappings-changes")
      .on("postgres_changes", { event: "*", schema: "public", table: "expense_mapping" }, fetchExpenseMappings)
      .subscribe();

    const stocksChannel = supabase
      .channel("stock-counts-cost-changes")
      .on("postgres_changes", { event: "*", schema: "public", table: "stock_counts" }, fetchStockCounts)
      .subscribe();
  ...
  const fetchExpenseMappings = async () => {
    const { data } = await supabase.from("expense_mapping").select("*");
    if (data) setExpenseMappings(data);
  };

  const fetchStockCounts = async () => {
    const { data } = await supabase.from("stock_counts").select("toy_id, closing_balance, date");
  ...
  const calculateFinishedToysForMonth = (monthStr: string) => {
    ...
    toys.forEach(toy => {
      const stocksForToy = stockCounts
        .filter(sc => sc.toy_id === toy.id && new Date(sc.date) <= monthEnd)
  ...
  const revenueHeads = expenseMappings
    .filter(m => m.is_revenue)
  ...
  const allExpenseHeads = Array.from(new Set(bankTransactions
    .map(t => normalizeHead(t.expense_head))
  ...
  const renderAmountCell = (key: string, month: string, idx: number) => {
    ...
    return (
      <TableCell key={idx} className="text-center py-1 text-xs">
        {amount !== 0 ? `₹${Number(Math.abs(amount)).toLocaleString('en-IN', { maximumFractionDigits: 0 })}` : "-"}
      </TableCell>
    );
  };
                ...
                <Table>
                  ...
                  <TableBody>
                    ...
                    <TableRow className="bg-muted/50 hover:bg-muted/60">
                      ...
                      {fyMonths.map((month, idx) => (
                        <TableCell key={idx} className="text-center py-1 text-xs font-medium">
                          ₹{getRevenueTotal(month).toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                        </TableCell>
                      ))}
                    ...
                    {expandedGroups["Revenue"] && revenueHeads.map(head => (
                      <TableRow key={head} className="hover:bg-muted/30">
                        ...
                        <TableCell className="text-center py-1 text-xs">
                          ₹{Math.abs(fyMonths.reduce((sum, m) => sum + getAmountForMonth(head, m), 0)).toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                        </TableCell>
                      </TableRow>
                    ))}
                      ...
                      return (
                        <React.Fragment key={groupName}>
                          <TableRow className="bg-muted/50 hover:bg-muted/60">
                            ...
                            {fyMonths.map((month, idx) => (
                              <TableCell key={idx} className="text-center py-1 text-xs font-medium">
                                ₹{Math.abs(getGroupTotal(groupName, month)).toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                              </TableCell>
                            ))}
                            <TableCell className="text-center py-1 text-xs font-bold">
                              ₹{Math.abs(fyMonths.reduce((sum, m) => sum + getGroupTotal(groupName, m), 0)).toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                            </TableCell>
                          </TableRow>

                          {expandedGroups[groupName] && heads.map(head => (
                            <TableRow key={head} className="hover:bg-muted/30">
                              <TableCell className="sticky left-0 z-10 bg-background pl-8 text-xs text-muted-foreground">
                                {head}
                              </TableCell>
                    ...
                    {plGroupedExpenses["Direct Expenses"] && (
                      <TableRow className="bg-primary/10 font-bold">
                        ...
                        {fyMonths.map((month, idx) => {
                          ...
                          return (
                            <TableCell key={idx} className={`text-center py-1 text-xs ${gross >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                              ₹{Math.abs(gross).toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                            </TableCell>
                          );
                        })}
                    ...
                    {plGroupedExpenses["Operating Cost"] && (
                      <TableRow className="bg-primary/10 font-bold">
                        ...
                        {fyMonths.map((month, idx) => {
                          ...
                          return (
                            <TableCell key={idx} className={`text-center py-1 text-xs ${net >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    ...
                    {Object.keys(otherGroupedExpenses).length > 0 && (
                      <>
                        ...
                        {Object.entries(otherGroupedExpenses).map(([groupName, heads]) => (
                          <React.Fragment key={groupName}>
                            <TableRow className="bg-muted/30 hover:bg-muted/40">
                              ...
                              {fyMonths.map((month, idx) => (
                                <TableCell key={idx} className="text-center py-1 text-xs text-muted-foreground">
                                  ₹{Math.abs(getGroupTotal(groupName, month)).toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                                </TableCell>
                              ))}
                            ...
                            {expandedGroups[groupName] && heads.map(head => (
                              <TableRow key={head} className="hover:bg-muted/20">
                                <TableCell className="sticky left-0 z-10 bg-background pl-8 text-xs text-muted-foreground/70">
                                  {head}
                                </TableCell>
                                {fyMonths.map((month, idx) => (
                                  <TableCell key={idx} className="text-center py-1 text-xs text-muted-foreground/70">
                                    {getAmountForMonth(head, month) !== 0 ? `₹${Number(Math.abs(getAmountForMonth(head, month))).toLocaleString('en-IN', { maximumFractionDigits: 0 })}` : "-"}
                                  </TableCell>
                                ))}
